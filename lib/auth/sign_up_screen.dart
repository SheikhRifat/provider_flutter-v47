...
import 'package:handyman_provider_flutter/auth/face/face_detector_screen.dart';
      ...
      children: [
        ...
        AppTextField(
          ...
          buildCounter: (_, {required int currentLength, required bool isFocused, required int? maxLength}) {
            return TextButton(
              child: Text(languages.lblChangeCountry, style: primaryTextStyle(size: 14)),
              onPressed: () {
                changeCountry();
              },
            );
          ...
          await authService.signUpWithEmailPassword(userData: userRegisterData.data!).then((value) {
            if (value) {
              if (userRegisterData.data!.status.validate() == 0) {
                ...
                push(FaceDetectorScreen(title: 'Face Verification',), isNewTask: true, pageRouteAnimation: PageRouteAnimation.Fade);
              }
            }
          }).catchError((e) {
            toast(languages.lblLoginAgain);
            FaceDetectorScreen(title: 'Face Verification',).launch(context, isNewTask: true);
          });
        }).catchError((e) {
          appStore.setLoading(false);
