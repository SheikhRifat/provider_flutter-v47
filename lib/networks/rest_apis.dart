...
Future<UserInfoResponse> getUserDetail(int id) async {
  return UserInfoResponse.fromJson(await handleResponse(await buildHttpResponse('user-detail?id=$id', method: HttpMethodType.GET)));
}
...
Future<DashboardResponse> providerDashboard() async {
  try {
    DashboardResponse data = DashboardResponse.fromJson(await handleResponse(await buildHttpResponse('provider-dashboard', method: HttpMethodType.GET)));
    ...
    if (data.appDownload != null) {
      setValue(PROVIDER_PLAY_STORE_URL, data.appDownload!.providerPlayStoreUrl.validate());
      setValue(PROVIDER_APPSTORE_URL, data.appDownload!.providerAppstoreUrl.validate());
    }

    appStore.setNotificationCount(data.notificationUnreadCount.validate());

    if (data.privacyPolicy != null && data.privacyPolicy!.value.validate().isNotEmpty) {
      appStore.setPrivacyPolicy(data.privacyPolicy!.value.validate());
    } else {
      appStore.setPrivacyPolicy(PRIVACY_POLICY_URL);
    }

    if (data.termConditions != null && data.termConditions!.value.validate().isNotEmpty) {
      appStore.setTermConditions(data.termConditions!.value.validate());
    } else {
      appStore.setTermConditions(TERMS_CONDITION_URL);
    }

    if (data.inquiryEmail.validate().isNotEmpty) {
      appStore.setInquiryEmail(data.inquiryEmail.validate());
    } else {
...
Future<HandymanDashBoardResponse> handymanDashboard() async {
  try {
    HandymanDashBoardResponse data = HandymanDashBoardResponse.fromJson(await handleResponse(await buildHttpResponse('handyman-dashboard', method: HttpMethodType.GET)));
